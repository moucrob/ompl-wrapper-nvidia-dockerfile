...
#https://gitlab.com/nvidia/opengl/blob/ubuntu16.04/glvnd/runtime/Dockerfile
...
# my image in which RViz is 2fps and ffmpeg recording isn't VLC readable
# fetched thanks to removing the bin bash (nop) and all these stuff + reordered
# the lines from bottom to top https://www.browserling.com/tools/reverse-lines,
# the output of that command in host' shell:
# docker history --format "{{.CreatedBy}}" --no-trunc=true motion-planner-wrapper_benchmarking:latest > motion-planner-wrapper_benchmarking-DOCKERFILE
################################################################################
